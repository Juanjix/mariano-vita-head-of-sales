...
function Chapter({ n, tag, children }: { n: number; tag: React.ReactNode; children: React.ReactNode }) {
  return (
    <div data-case-chapter className="relative grid-12 gap-y-4 border-t rule py-[clamp(2.25rem,5vw,4rem)]">
      <span data-case-rule aria-hidden="true" className="absolute inset-x-0 -top-px h-px origin-left bg-rust" />
      <p data-reveal="fade" className="col-span-12 flex items-baseline gap-4 md:col-span-3">
        <span data-case-num className="font-serif text-4xl leading-none text-rust md:text-5xl">
          {pad(n)}
        </span>
      ...
      <section aria-labelledby="case-title" className="bg-paper">
        <div className="shell pt-[clamp(6rem,13vw,12rem)] pb-[clamp(5rem,11vw,10rem)]">
          ...
          <div className="mt-[clamp(4rem,9vw,8rem)]">
            ...
            {/* The 15-day cycle */}
            <div data-case-chapter className="relative grid-12 gap-y-8 border-t rule py-[clamp(2.5rem,5vw,4rem)]">
              <span data-case-rule aria-hidden="true" className="absolute inset-x-0 -top-px h-px origin-left bg-rust" />
