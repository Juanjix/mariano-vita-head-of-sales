  ...
  return (
    ...
    >
      ...
      <ol className="relative mt-[clamp(3rem,7vw,6rem)] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        <span data-reveal="rule" aria-hidden="true" className="absolute inset-x-0 top-0 h-px bg-ink" />
        {numbers.stats.map((stat, i) => {
          ...
          return (
            ...
            >
              <span className="hidden label text-mute num sm:block">({String(i + 1).padStart(2, "0")})</span>
              <p className="font-serif text-[clamp(3.6rem,17vw,5.5rem)] leading-[0.85] sm:mt-10 sm:text-[clamp(4.5rem,9vw,7rem)] lg:mt-16 lg:text-[clamp(4.25rem,8.2vw,9.5rem)]">
                {unit && <span className="mr-2 inline-block pt-[0.9em] align-top label text-rust sm:mr-3">{unit}</span>}
                <CountUp figure={figure} />
              </p>
              <p className="max-w-[10rem] text-right text-[0.95rem] leading-snug text-ink/75 sm:mt-5 sm:max-w-[16rem] sm:text-left">
                <T v={stat.label} />
              </p>
            </li>
          );
      ...
      <p data-reveal="fade" className="mt-8 label text-mute">
        <T v={numbers.note} />
      </p>
    </section>
  );
