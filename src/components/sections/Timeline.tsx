    ...
    <TimelineMotion>
      <section aria-labelledby="timeline-title" className="bg-paper">
        <div className="shell pt-[clamp(6rem,13vw,12rem)] pb-[clamp(3rem,6vw,5rem)]">
          ...
          {/* Chronology */}
          <div data-tl-pin className="relative mt-[clamp(4rem,9vw,8rem)]">
            {/* Horizontal track (desktop): ink base + rust progress */}
            <span aria-hidden="true" className="absolute inset-x-0 top-0 hidden h-px bg-ink/25 lg:block">
              <span data-tl-fill className="absolute inset-0 origin-left bg-ink" />
            </span>
            {/* Vertical spine progress (mobile/tablet) */}
            <span aria-hidden="true" className="absolute top-0 bottom-0 left-0 w-px lg:hidden">
              <span data-tl-spine className="absolute inset-0 origin-top bg-rust" />
            </span>
            <ol className="relative lg:grid lg:grid-cols-4 lg:gap-x-[clamp(1.5rem,2.4vw,2.5rem)]">
              {t.chapters.map((ch, i) => (
                <li
                  key={ch.from}
                  data-tl-chapter
                ...
                >
                  ...
                  <p className="flex items-baseline gap-3 lg:block">
                    ...
                    <span
                      data-tl-year
                      className="block font-serif text-[clamp(3.25rem,15vw,5rem)] leading-[0.82] num lg:text-[clamp(3.5rem,6.2vw,7rem)]"
                    >
                      {ch.from}
                    </span>
            ...
            <div data-tl-handoff className="relative flex items-center gap-4 pl-7 sm:pl-10 lg:mt-16 lg:pl-0">
              <span
                aria-hidden="true"
                className="absolute top-1/2 -left-[4.5px] h-2 w-2 -translate-y-1/2 rounded-full border border-rust bg-paper lg:hidden"
              />
              <span
                data-tl-handoff-rule
                aria-hidden="true"
                className="hidden h-px flex-1 origin-left bg-rust lg:block"
              />
              <p data-tl-handoff-text className="flex items-center gap-3 label text-rust">
                <span>
